package org.nastation.components;

import com.vaadin.flow.component.html.Span;
import org.nastation.common.util.CompUtil;
import org.nastation.components.style.css.lumo.BadgeColor;
import org.nastation.components.style.css.lumo.BadgeShape;
import org.nastation.components.style.css.lumo.BadgeSize;

import java.util.StringJoiner;

import static org.nastation.components.style.css.lumo.BadgeShape.PILL;

public class Badge extends Span {

	public Badge(String text) {
		this(text, BadgeColor.NORMAL);
	}

	public Badge(String text, BadgeColor color) {
		super(text);
		CompUtil.setTheme(color.getThemeName(), this);
	}

	public Badge(String text, BadgeColor color, BadgeSize size, BadgeShape shape) {
		super(text);
		StringJoiner joiner = new StringJoiner(" ");
		joiner.add(color.getThemeName());
		if (shape.equals(PILL)) {
			joiner.add(shape.getThemeName());
		}
		if (size.equals(BadgeSize.S)) {
			joiner.add(size.getThemeName());
		}
		CompUtil.setTheme(joiner.toString(), this);
	}

}
