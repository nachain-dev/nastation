package org.nastation.components.navigation.bar;
...
import org.nastation.components.navigation.tab.NaviTab;
import org.nastation.components.navigation.tab.NaviTabs;
...
public class AppBar extends FlexBoxLayout {
	...
	private void initMenuIcon() {
		menuIcon = CompUtil.createTertiaryInlineButton(VaadinIcon.MENU);
		menuIcon.addClassName(CLASS_NAME + "__navi-icon");
		//menuIcon.addClickListener(e -> MainLayout.get().getNaviDrawer().toggle());
		CompUtil.setAriaLabel("Menu", menuIcon);
		CompUtil.setLineHeight("1", menuIcon);
	}

	private void initContextIcon() {
		contextIcon = CompUtil
				.createTertiaryInlineButton(VaadinIcon.ARROW_LEFT);
		contextIcon.addClassNames(CLASS_NAME + "__context-icon");
		contextIcon.setVisible(false);
		CompUtil.setAriaLabel("Back", contextIcon);
		CompUtil.setLineHeight("1", contextIcon);
	}
	...
	private void initTabs(NaviTab... tabs) {
		addTab = CompUtil.createSmallButton(VaadinIcon.PLUS);
	...
	public Button addActionItem(VaadinIcon icon) {
		Button button = CompUtil.createButton(icon, ButtonVariant.LUMO_SMALL,
